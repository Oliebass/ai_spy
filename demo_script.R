...
file_copy(ai_output_files, sub("test_data", "demo/Incoming Images", ai_output_files))
file_copy(image_files, sub("test_data", "demo/Incoming Images", image_files))

detections <- read.csv("demo/Incoming Images/results_files.csv", header = TRUE)
detections$absolute_path <- paste0(getwd(), '/demo/Incoming Images')
write.csv(detections, "demo/Incoming Images/results_files.csv", row.names = FALSE)
classifications <- read.csv("demo/Incoming Images/results_detections.csv", header = TRUE)
classifications$absolute_path <- paste0(getwd(), '/demo/Incoming Images')
write.csv(classifications, "demo/Incoming Images/results_detections.csv", row.names = FALSE)
